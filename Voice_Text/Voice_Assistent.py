    ...
    if provera == True:
        ...
        if provera_2 == True:
            ...
            if konvert == 'browser':
                ...
                # try:
                #     r = requests.get(url)
                #     href = re.compile(r'link rel = "canonical" href = ".*"/>')
                #     novi_url = href.search(r.text).groups()[0]

                #     soup =  BeautifulSoup(r.tetx, "html.parser")
                #     novi_url = soup.find("link", rel = "canonical")["href"]
                #     print(novi_url)
                # except:
                #     print("greska")
